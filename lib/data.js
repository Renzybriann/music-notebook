...
function normalizeTrimEnd(end) {
  if (end == null || !Number.isFinite(Number(end))) return null;
  const n = Number(end);
  return n > 0.25 ? n : null;
}

function normalizeTrimStart(start) {
  if (start == null || !Number.isFinite(Number(start))) return null;
...
export async function uploadSong(userId, { title, durationDisplay, durationSeconds, isReleased, notes, audioFile, startTimeSeconds, endTimeSeconds }) {
  ...
  const startNorm = normalizeTrimStart(startTimeSeconds);
  let endNorm = normalizeTrimEnd(endTimeSeconds);
  if (endNorm != null && startNorm != null && endNorm <= startNorm) endNorm = null;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      notes: notes || null,
      start_time_seconds: startNorm,
      end_time_seconds: endNorm,
    })
    .select(
